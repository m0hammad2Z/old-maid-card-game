...
public class Player extends Thread{
    ...
    private Card getMatchingCard(Card card) {
        for (Card currentCard : hand) {
            ...
            if (card.isMatchingPair(currentCard)) {
                return currentCard;
            }
        }
        return null;
    ...
    public void run() {
        ...
        while (game.isRunning() && !hand.isEmpty()) {
            synchronized (game){
                while(game.getCurrentPlayer().playerId != this.playerId){
                    try {
                        game.wait();
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                ...
                if(game.getWinner() == null){
                    ...
                    System.out.println("Player " + playerId + " have " + hand.size() + " cards");
                }
